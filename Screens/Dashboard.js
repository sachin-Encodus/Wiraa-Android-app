...
export default class Dashboard extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    try {
     await fetch("http://demo.wiraa.com/api/Users/GetUsers?Id=" + userId, {
      method: "GET",
      //Request Type
    })
      ...
      .then((responseJson) => {
        this.setState({
         ...
         AsyncStorage.setItem("userPrfileId", JSON.stringify(responseJson.usersProfile.usersProfileID));
        AsyncStorage.setItem("userType", JSON.stringify(responseJson.userType));
    ...
    // this.setState({ isLoading: true });
    // Get Posts
    await this.getPosts(await userProfileId, this.state.count);
    //GET ALL USERS
  ...
  getProRequestStatus=async(userId)=>{
     this.setState({proCount:this.state.proCount+1})
    ...
    .then((responseJson)=>{
      ...
      if (this.state.myKeys === true) {
      AsyncStorage.setItem('Appdata', 'sachin')
    }else{
      // console.log("==================>>>>>>>>.false");
    }
    });
          ...
          // console.log('---------9999999888888',responseJson.status)
        }
      ...
      // console.log('response get pro request status--->>>',responseJson)
    })
    .catch((error)=>{
      alert(error)
  ...
  getPosts = async(userProfileId, count) => {
  ...
  getMorePost = (userProfileId, count) => {
    ...
    fetch(
      ...
      .then((responseJson) => {
        ...
        this.setState({ refreshing: false });
        this.setState({ footerLoader: false });
        let posts = [...this.state.posts,...responseJson];
        this.setState({ posts: posts})
        // responseJson.map((item) => {
        //   console.log('=0099099----->>',item.postImagesList.imageURL)
        //   posts.push({
        //     fkUserProfileId: item.fkUserProfileID,
        //     id: item.$id,
        //     postId: item.postID,
        ...
        //     // imageURL:
        //     //   item.postImagesList[0] !== undefined
        //     //     ? item.postImagesList[0].imageURL.split("~")
        //     //     : "",
        //     postDate: item.postDate,
        //     postComments: item.postComments,
        //   });
  ...
  addLike = async (postId) => {
    await fetch("http://demo.wiraa.com/api/Post/AddRemoveLike", {
      ...
      .then((response) => response.json())
      .then((responseJson) => {
        // console.log('add like----->>--------',this.state.posts)
      ...
      <View>
        ...
        >
          <View style={{ flex: 1, flexDirection: "row" }}>
            ...
            />
            <View style={{ marginTop: 8,   width: widthPercentageToDP(40)}}>
              <View
                style={{
                  flexDirection: "row",
  ...
  renderArticles = (item,index) => {
    // console.log('item render artical------',item)
    // const date = item.postDate.split("T");
    const imageURL =  
          item.postImagesList !== undefined
            ? item.postImagesList[0].imageURL.split("~")
            : "" 
            // item.imageURL.split("~");
              ...
              {this.state.userType === 3 ? (
                ...
                >
                  <View style={{ flexDirection: "row",marginTop:"auto",marginBottom:"auto" }}>
                    <Text style={[styles.btntxt,{fontSize:widthPercentageToDP(5)} ]}> Post Your Portfolio </Text>
                  </View>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity
                  style={styles.btn}
                  onPress={() => this.state.proStatus === "PENDING" ? alert('Congratulations. \n Your account has been sent for approval. \n Please Wait!') : this.props.navigation.navigate("BusinessUser") }
                >
                  <View style={{ flexDirection: "row",marginTop:"auto",marginBottom:"auto" }}>
